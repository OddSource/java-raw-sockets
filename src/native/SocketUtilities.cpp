/*
 * RawSocketImpl.c from RawSockets modified Wednesday, June 29, 2011 10:58:53 CDT (-0500).
 *
...
/*
 * Class: net_nicholaswilliams_java_net_socket_SocketUtilities
 * Method: getProtocolByName
 * Signature: (Ljava/lang/String;)Lnet/nicholaswilliams/java/net/socket/Protocol;
 *
 ...
 * Signature: (I)Lnet/nicholaswilliams/java/net/socket/Protocol;
 *
 ...
 * Signature: ()[Lnet/nicholaswilliams/java/net/socket/Protocol;
 *
	...
	(JNIEnv *environment, jclass SocketUtilities)
{
	...
	jclass Protocol = environment->FindClass("net/nicholaswilliams/java/net/socket/Protocol");
	if(Protocol == 0)
	{
		environment->ThrowNew(
			environment->FindClass("java/lang/ClassNotFoundException"),
			"Could not find class net.nicholaswilliams.java.net.socket.Protocol."
		);
		return NULL;
	}
...
static jobject getProtocolObjectByProtocolStruct(JNIEnv *environment, struct protoent *protocol)
{
	...
	if(constructor == 0)
	{
		environment->ThrowNew(
			...
			"Could not find method <init>(String, String[], int) in class net.nicholaswilliams.java.net.socket.Protocol."
		);
		return NULL;
	}
