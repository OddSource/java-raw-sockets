...
JNIEXPORT void JNICALL Java_net_nicholaswilliams_java_net_socket_RawSocketImpl_nativeStaticInitialize(JNIEnv *, jclass)
{
#if defined(_WIN32)
...
JNIEXPORT void JNICALL Java_net_nicholaswilliams_java_net_socket_RawSocketImpl_nativeStaticShutdown(JNIEnv *, jclass)
{
#if defined(_WIN32)
	WSACleanup();
#endif
...
static void raiseError(JNIEnv * environment, std::string error, std::string message)
{
	if(error.empty() || error.length() == 0)
		error = "net/nicholaswilliams/java/net/socket/exception/RawSocketException";
